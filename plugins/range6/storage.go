...
var (
	...
	// caller wanted gone is already gone, which is not a failure.
...
const (
	...
	// the disk, counted from when the change was queued so that it covers
	// the wait in the queue too. A write that takes seconds is one the
	// client has already retransmitted past, so the binding is refused
	...
	// a client its binding, and the retries run inside the same writeTimeout
...
// bindingWrite is one queued change to the lease database. The description
// is put together only when a write fails, which is why the DUID and the
// address travel alongside the arguments instead of as one string.
