...
func (s *pluginState) Handler6(ctx context.Context, req, resp dhcpv6.DHCPv6) (dhcpv6.DHCPv6, bool) {
	...
	// A mapping is a fixed assignment, so this is the answer: replace any
	// IA_NA already in the reply and end the chain, the way Handler4 does.
	// Two IA_NA options carrying the same IAID, which is what an AddOption
	// here plus a range6 either side of this plugin produced, is not a reply
	// RFC 8415 section 21.4 allows.
