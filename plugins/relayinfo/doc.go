...
// On DHCPv6 it adds an IA_NA for the IAID the client asked with and ends the
// chain there too, because a range6 behind it would otherwise add a second
// IA_NA for the same IAID, which RFC 8415 does not allow.
//
...
// # Placement
//
// A matched request ends the chain on both families, so relayinfo goes last
// among the plugins that hand out addresses, with the option plugins ahead of
// it. A dropped request ends the chain as well, so no later plugin answers it
// either. Only a request carrying relay information is ever dropped. Drops are logged
