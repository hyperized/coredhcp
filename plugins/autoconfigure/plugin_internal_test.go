...
func TestSetup4(t *testing.T) {
	cases := []struct {
		...
		{name: "unknown value", args: []string{"bogus"}, wantErr: `"bogus" is not an autoconfigure value`},
		{name: "too many arguments", args: []string{"1", "extra"}, wantErr: "autoconfigure takes at most one"},
		{name: "unknown value takes precedence over too many arguments", args: []string{"bogus", "extra"}, wantErr: `"bogus" is not an autoconfigure value`},
	}
