...
// Edit cmd/coredhcp-generator/coredhcp-tui.go.template instead. This is a
// separate module, so the directive below names the generator by import path
// rather than by the relative path the coredhcp template can use.

//go:generate go run github.com/coredhcp/coredhcp/cmd/coredhcp-generator -t ../coredhcp-generator/coredhcp-tui.go.template -f ../coredhcp-generator/core-plugins.txt -o main.go
...
func run(w io.Writer) error {
	...
	if *flagLogFile != "" {
		...
		if err := logger.WithFile(*flagLogFile); err != nil {
			return fmt.Errorf("%w; point --logfile at a path the server's user can write, or leave it out to keep the log in the interface's log pane only", err)
		}
	}
	...
	for _, plugin := range desiredPlugins {
		if err = plugins.RegisterPlugin(plugin); err != nil {
			// A nil entry is one of the errors, so the name is left to err.
			return fmt.Errorf("cannot register a built-in plugin: %w; nothing an operator can change causes this, so report it as a bug", err)
		}
	}
